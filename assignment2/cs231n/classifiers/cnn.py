...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # 默认stride, padding等参数已知，具体见loss函数
        # 图像经过卷积和maxpool特征图被下采样2倍，长和宽都缩小2倍
    ...
    def loss(self, X, y=None):
        ...
        cls_out, cls_cache = affine_forward(hid_out, W3, b3)
        scores = cls_out                    
        ...
        loss += 0.5 * self.reg * (np.sum(W1 ** 2) + np.sum(
                 W2 ** 2) + np.sum(W3 ** 2))
        
        dx, grads['W3'], grads['b3'] = affine_backward(ds, cls_cache)
        grads['W3'] += self.reg * W3

        dx, grads['W2'], grads['b2'] = affine_relu_backward(dx, hid_cache)
